...
const StudentNavbar: React.FC<StudentNavbarProps> = ({
  ...
  const latestSearchChange = useRef(onSearchChange);
  const latestSearchResults = useRef(onSearchResults);

  useEffect(() => {
    latestSearchChange.current = onSearchChange;
  }, [onSearchChange]);

  useEffect(() => {
    latestSearchResults.current = onSearchResults;
  }, [onSearchResults]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      latestSearchChange.current?.(raw);

      if (!query) {
        latestSearchResults.current?.(raw, null);
        return;
      }

      try {
        const res = await studentDashboardAPI.searchInstitutionCourses(
          query,
          controller.signal
        );

        if (res.success && Array.isArray(res.data)) {
          latestSearchResults.current?.(raw, res.data);
        } else {
          latestSearchResults.current?.(raw, null);
        }
      } catch (err) {
